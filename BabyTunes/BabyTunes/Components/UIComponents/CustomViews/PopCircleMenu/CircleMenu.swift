...
open class CircleMenu: UIButton {
    ...
    open var showDelay: Double = 0.0
    /// Highlighted border Color
    open var highlightedBorderColor: UIColor = UIColor.color(255, green: 22, blue: 93, alpha: 1.0)
    /// Normal border Color
    ...
    fileprivate func createButtons() -> [CircleMenuButton] {
        ...
        for index in 0..<self.buttonsCount {
            ...
            let button = Init(CircleMenuButton(size: size, circleMenu: self, distance:distance, angle: angle, index: index)) {
                $0.tag = index
                $0.addTarget(self, action: #selector(CircleMenu.buttonHandler(_:)), for: UIControlEvents.touchDragExit)
                $0.alpha = 0
            }
    ...
    internal func buttonHandler(_ sender: UIButton) {
        ...
        let dispatchTime: DispatchTime = DispatchTime.now() + Double(Int64(min(duration, 0.2) * Double(NSEC_PER_SEC))) / Double(NSEC_PER_SEC)
